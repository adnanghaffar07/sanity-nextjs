...
async function addEmailToAirtable(emailData: {
  ...
  const response = await fetch(
    `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/tbl6At5GiNNcRynH5`, // Use table ID
    {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        fields: {
          fldg8mdtACGYOg1hf: emailData.Name, // Name field ID
          fldIBf9VVLXm9VcLd: emailData.Email, // Email field ID
          fldWtLqJCB7LeQt7W: emailData.Subject, // Subject field ID
          fldVlHFYEaLPBpVrU: emailData.Body, // Body field ID
          fldReminderTimestamp: emailData.ReminderTimestamp, // Add timestamp
        },
      }),
    }
  );
