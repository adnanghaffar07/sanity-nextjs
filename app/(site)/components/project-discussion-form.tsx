...
export default function ProjectDiscussionContainer() {
    const [formData, setFormData] = useState({
        name: { value: '', placeholder: 'Your Name', pattern: '', errormessage: 'Your name is required' },
        contactNumber: { value: '', placeholder: 'Your Contact Number', pattern: '', errormessage: 'Your contact number is required' },
        email: { value: '', placeholder: 'Your Email', pattern: '', errormessage: 'Your email is required' },
    ...
    const handleChange = (e:any) => {
        ...
        setFormData((prevData: any) => ({ ...prevData, [name]: { ...prevData[name as keyof typeof formData], value } }));
    };

    const handleClick = () => {
        const isValidForm = Object.values(formData).every(field => field.value !== '' );
